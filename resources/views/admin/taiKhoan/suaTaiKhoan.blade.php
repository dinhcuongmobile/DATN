...
@section('containerAdmin')
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <form action="{{ route('tai-khoan.sua', $taiKhoan->id) }}" method="post" class="form">
                    @csrf
                    @method('put')
                    <div class="row mb-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('email') is-invalid @enderror" id="email"
                                name="email" placeholder="VD: example@gmail.com..." value="{{ $taiKhoan->email }}">
                            @error('email')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>  
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('dia_chi') is-invalid @enderror" id="dia_chi"
                                name="dia_chi" placeholder="Nhập địa chỉ..." value="{{ $taiKhoan->dia_chi }}">
                            @error('dia_chi')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control @error('vai_tro_id') is-invalid @enderror" id="sel1" name="vai_tro_id">
                                ...
                                @foreach ($vaiTro as $item)
                                    <option value="{{ $item->id }}" {{ $taiKhoan->vai_tro_id == $item->id ? 'selected': '' }}>{{ $item->vai_tro }}</option>
                                @endforeach
                            </select>
                            @error('vai_tro_id')
                                <div class="text-danger">{{ $message }}</div>
