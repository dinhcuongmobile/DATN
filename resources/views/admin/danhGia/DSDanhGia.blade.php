            ...
            <div class=" float-right">
                <form method="GET">
                    <div class="input-group">
                        <input type="text" class="form-control" name="kyw" placeholder="Tìm kiếm theo Họ và Tên...">
            ...
            <form action="" method="post">
                ...
                <div class="float-left">
                    ...
                    <button type="button" class="btn btn-secondary btn-sm" onclick="bochontatca()">Bỏ chọn tất
                        cả</button>
                    <button  onclick="return confirm('Bạn chắc chắn muốn ẩn các mục đã chọn?')"
                            type="submit" class="btn btn-secondary btn-sm">Ẩn các mục đã chọn</button>
                </div>
        </div>
        <div class="card-body">
                        ...
                        @if (count($danhGias) > 0)
                            @foreach($danhGias as $item)
                                @if ($item->noi_dung != null)
                                    @php
                                        $ratingStars = str_repeat('<i class="fa-solid fa-star"></i>', $item->so_sao) .
                                    ...
                                    <tr>
                                        ...
                                        <td class="col-3 align-middle">
                                            <a href="{{route('san-pham.san-pham-bien-the',$item->san_pham_id)}}">{{$item->sanPham->ten_san_pham}}</a>
                                        </td>
                                        <td class="col-3 align-middle">{{ $item->noi_dung }}</td>
                                        <td class="col-2 align-middle">{!! $ratingStars !!}</td>
                                        <td class="col-1 align-middle">{{ $item->created_at }}</td>
                                        <td class="col-1 align-middle">
                                            {{-- @if ($item->trang_thai)
                                            <a href="{{ route('admin.item.toggleHide', $item->id) }}">
