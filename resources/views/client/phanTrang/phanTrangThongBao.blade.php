<ul class="pagination">
    {{-- Nút "Trước" --}}
    <li class="{{ $thong_baos->onFirstPage() ? 'disabled' : '' }}">
        <a class="prev" href="javascript:void(0);" data-page="{{ $thong_baos->currentPage() - 1 }}">
    ...
    {{-- Hiển thị số trang --}}
    @for ($i = 1; $i <= $thong_baos->lastPage(); $i++)
        <li>
            <a class="{{ $i == $thong_baos->currentPage() ? 'active' : '' }}"
            href="javascript:void(0);" data-page="{{ $i }}">{{ $i }}</a>
        </li>
    @endfor

    {{-- Nút "Tiếp" --}}
    <li class="{{ $thong_baos->hasMorePages() ? '' : 'disabled' }}">
        <a class="next" href="javascript:void(0);" data-page="{{ $thong_baos->currentPage() + 1 }}">
